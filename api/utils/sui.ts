...
export const getSigner = () => {
  ...
  const keystore = JSON.parse(readFileSync(path.join(homedir(), ".sui", "sui_config", "sui.keystore"), "utf8"));
  ...
  for (const priv of keystore) {
    ...
    const pair = Ed25519Keypair.fromSecretKey(new TextEncoder().encode(priv).slice(1));
...
export const publishPackage = async ({ packagePath, network, exportFileName = "contract" }: { packagePath: string; network: Network; exportFileName: string }) => {
