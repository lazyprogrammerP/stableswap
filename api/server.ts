...
import { formatPaginatedResponse, parsePaginationForQuery, parseWhereStatement, WhereParam, WhereParamTypes } from "./utils/api-queries";
...
app.get("/locked", async (req, res) => {
  ...
  try {
    const locked = await db.locked.findMany({
...
app.listen(8000, () => console.log(`🚀 Server ready at: http://localhost:3000`));
