...
import { getActiveAddress, getClient, signAndExecute } from "../utils/sui";
...
const createEscrows = async (total: number) => {
  ...
  if (!lockedObjects || !ownedBears || lockedObjects.length < total || ownedBears.length < total) throw new Error(`please run 'ts-node create-demo-data.ts' first, with at least ${total} bears.`);
  ...
  for (const tuple of tuples) {
    ...
    txb.moveCall({
      ...
      arguments: [txb.object(tuple.bear.objectId), txb.pure.address(tuple.locked.key), txb.pure.address(getActiveAddress())],
      typeArguments: [tuple.bear.type!],
    });
  }

  const res = await signAndExecute(txb, CONFIG.NETWORK);
  if (!res.objectChanges || res.objectChanges.length === 0) throw new Error("Something went wrong while creating escrows.");
