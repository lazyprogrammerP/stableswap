...
import { ACTIVE_NETWORK, getActiveAddress, signAndExecute } from "../utils/sui";
...
const createDemoLockedObjects = async (totalBears: number) => {
  ...
  if (!res.objectChanges || res.objectChanges.length === 0) throw new Error("Something went wrong while creating demo bears & locked objects.");
